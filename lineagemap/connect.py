"""Native cognis-connect emit for lineagemap — forward findings to any platform.

Maps lineagemap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    lineagemap ... --format json | lineagemap-emit --to stix
    lineagemap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a lineagemap record toward the Finding contract before normalize().
